...
fn main() -> Result<()> {
    ...
    let wasm_file = "./examples/wasm_file/fibonacci.wat";
    let func_name = "fibonacci";
    let func_args = [];
...
fn load_wasm_func(
    ...
    let engine = zk_wasmi::Engine::default();
    let mut store = zk_wasmi::Store::new(&engine, ());
    let module = zk_wasmi::Module::new(&engine, &mut &wasm_bytes[..]).map_err(|error| {
        anyhow!("failed to parse and validate Wasm module {wasm_file}: {error}")
    })?;
    let mut linker = <zk_wasmi::Linker<()>>::new();
    let instance = linker
...
///
/// [`Module`]: [`wasmi::Module`]
fn exported_funcs(module: &zk_wasmi::Module) -> Vec<(&str, FuncType)> {
    module
        .exports()
        .filter_map(|export| {
...
///
/// [`Module`]: [`wasmi::Module`]
fn display_exported_funcs(module: &zk_wasmi::Module) -> String {
